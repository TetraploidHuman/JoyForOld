...
object AgentActionSet {
    ...
    const val ID_WECHAT_SEND_IM_MESSAGE = ImSendMessageActionSet.ID
    ...
    fun descriptionsForPrompt(): String {
        ...
        - run_action_set: 一次展开**固定多步动作组**（本地执行固定 UI 路径，动作组进行中尽量不再调用主规划、不反复塞整棵 UI 树）。
          ...
          **仅当**用户意图明确、参数齐全、且动作组比逐步 tap/click 更合适时调用；否则用 open_app/click/type 逐步规划。
          · target_text: 动作组 ID
          · $ID_WECHAT_SEND_IM_MESSAGE（微信专用发消息）：用户明确说微信/用微信/发微信，且联系人+正文齐全时**优先**调用；input_text=联系人名，message=消息正文（必填）
            示例：{"action":"run_action_set","target_text":"$ID_WECHAT_SEND_IM_MESSAGE","input_text":"大女儿","message":"今晚回家吃饭"}
            TIM/QQ/其它 IM 或未点名微信：不要用本动作组，改用 open_app + 逐步 click/type/send。
